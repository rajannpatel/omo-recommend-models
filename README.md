...
- **Bounded Concurrent Execution**: To balance throughput against CPU/memory strain and provider rate limits, provider probes are run with a bounded scheduler (default: up to the detected CPU parallelism concurrent `opencode` subprocesses globally — via `os.availableParallelism()`, falling back to `os.cpus().length`, then to 2 only if both detection sources are unusable — at most 2 concurrent probes per individual paid provider, and at most 1 concurrent probe across free/local/CLI-backed candidates). Availability is rechecked immediately before each dispatch so quota/rate-limit state discovered mid-run is honored without spawning stale subprocesses. A model-scope rate-limited probe result cools its provider down for the remainder of the run (remaining queued refs for that provider are skipped) without invalidating that provider's already-confirmed successes, distinguishing a temporary throttle from full quota exhaustion.
...
* **Cloud inventory**
    ...
    For OpenRouter, the tool efficiently excludes policy-disallowed models before any slow probe: it fetches the authenticated user-effective model allowlist (`https://openrouter.ai/api/v1/models/user`) and drops OpenRouter refs the account cannot use; it also removes OpenRouter refs already recorded in the persisted policy-exclusion cache from a prior guardrail/policy probe failure. The API key for this check is resolved from `OPENROUTER_API_KEY`/`OPENROUTER_BEARER` first, then falls back to the credential `opencode auth login` already stored in `opencode`'s own `auth.json` (API key or OAuth access token), so no extra setup is needed when that stored credential is present and valid (an expired, unrefreshed OAuth token still falls back to the "unavailable" path). Both exclusions happen before `opencode models --verbose` enrichment and before any `say 1` probe, and the CLI prints how many models were excluded (or that the live allowlist was unavailable and cached exclusions were checked instead) at each step.
* **Local inventory**

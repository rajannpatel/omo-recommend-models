...
export function createAiMatcher(fallbackChain, probeState) {
   ...
   */
  async function findClosestMatch(requirement, providerModelList, options = {}) {
   ...
   */
  async function tryAiMatchWithProvider(requirement, providerModelList, provider, model, variant) {
    try {
      // Build prompt for AI matching
      const prompt = buildAiPrompt(requirement, providerModelList, provider, model, variant);
   ...
   */
  function buildAiPrompt(requirement, providerModelList, provider, model, variant) {
...
Return only valid JSON with no additional text or markdown formatting.`;
   ...
   */
  async function callLlm(provider, modelRef, prompt) {
...
/**
 * Find the closest match using AI for the given requirement and provider model list.
 * This is a convenience function that creates an AI matcher with default parameters.
 * @param {Object} requirement - Model requirement object
 * @param {Array} providerModelList - Array of {modelId, metadata} objects
 * @param {Object} [options] - Options for AI matching
 * @returns {Promise<Object|null>} AI match result or null if no match found
 */
export async function aiFindClosestMatch(requirement, providerModelList, options = {}) {
  // This is a convenience function that creates a temporary AI matcher
  // In practice, this would be used by matchWithAI in model-matching.js
  const fallbackChain = [];
  const probeState = {};
  
  const matcher = createAiMatcher(fallbackChain, probeState);
  return matcher.findClosestMatch(requirement, providerModelList, options);
}
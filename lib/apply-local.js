 ...
 * Iterates cloudRecommendations and sets section.model and section.fallback_models.
...
/**
 * Install local models based on AI decisions.
...
export async function resolveInstallDecisions({
  ...
  const missingInstalls = toInstall.filter((d) => !installedNames.has(d.name));
  const installMode =
...
async function chooseInstallMode() {
  ...
  const answer = (await promptLine("Choose 1, 2, or 3 [3]: "))
    .toLowerCase()
    .trim();
  if (answer === "1" || answer === "y" || answer === "yes" || answer === "all") {
    return "all";
  }
  if (answer === "2" || answer === "p" || answer === "per" || answer === "choose") {
    return "per-model";
  }
  return "none";
}

...
function buildCommand() {
  return new Command()
    ...
    .option("--rebalance", "Perform algorithmic tier-chain restructuring")
    ...
    .option("--rebalance-apply", "Write restructuring changes in rebalance mode")
    .option("--no-rebalance-apply", "Do not write restructuring changes in rebalance mode")
...
export function parseCliOptions(rawArgs = process.argv.slice(2)) {
  ...
  parsed.rebalance = Boolean(opts.rebalance);
  ...
  parsed["rebalance-apply"] = opts.rebalanceApply === true;

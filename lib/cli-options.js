...
export function usage() {
  return [
    ...
    "      --flush-cache                  Clear cached policy-excluded model refs before probing",
    ...
    "      --free-config                Include zero-cost models in JSONC configuration",
    "      --no-free-config             Exclude zero-cost models from JSONC configuration",
...
export function parseCliOptions(rawArgs = process.argv.slice(2)) {
  ...
  parsed["flush-cache"] = Boolean(opts.flushCache);
  ...
  parsed.verbose = Boolean(opts.verbose);

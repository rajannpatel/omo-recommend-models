...
function createModelAllowedPredicate(availablePaidModels) {
  return ({ provider, model }) => {
    ...
    return availablePaidModels.has(`${provider}/${model}`);
  };
}
...
export async function selectRecommendation({
  ...
  try {
    if (!runOptions.useAiPanel) {
      ...
      const availablePaidModels = new Set(
        await inputs.paidProviderPrep.paidProbesPromise,
      );
      return {
        aiResult: finalizeRecommendation({
          ...
          result: createRuleBasedRecommendations({
            ...
            isModelAllowed: createModelAllowedPredicate(availablePaidModels),
          }),
        }),
...
async function selectPanel({
  ...
  const panelSelection = await selectPanelRecommendation({
    ...
    ensureProbesAwaited: inputs.paidProviderPrep.ensureProbesAwaited,

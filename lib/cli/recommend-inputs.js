...
function buildCloudLookup({ localOnlyFlag, paidProviderPrep }) {
  ...
  // Free opencode models are always available as fallback candidates unless
  ...
  // and AI ranking even when the opencode provider is not explicitly configured.
  if (cache?.models && (!cache.models.opencode || cache.models.opencode.length === 0)) {
    const freeModels = discoverFreeModels();
    if (freeModels.length > 0) {
      cache.models.opencode = freeModels.map((ref) => ({ id: ref.slice(ref.indexOf("/") + 1) }));
    }
  }

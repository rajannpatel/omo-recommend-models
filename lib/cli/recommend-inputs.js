...
import { createProgress, writeGroupSeparator, writeTopLevelLine } from "../display-utils.js";
...
async function buildCloudLookup({ ctx, localOnlyFlag, paidProviderPrep }) {
  if (localOnlyFlag) {
  ...
  writeTopLevelLine("◇  Checking live provider models...");
  writeGroupSeparator();
  const cache = paidProviderPrep?.initialCache || await loadProviderModels({ ctx });
  ...
  if (cache?.models) {
    const freeModels = discoverFreeModels({ ctx });
  ...
  cloudProgress.done(`${count} providers (live from \`opencode models\`)`);

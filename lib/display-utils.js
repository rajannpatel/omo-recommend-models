...
export function defaultPanelModels(config, cloudLookup = null, options = {}) {
  ...
  const preferFn = typeof options._preferDetectedCliPanelModels === "function" ? options._preferDetectedCliPanelModels : (refs, _cfg, _cl, max) => refs.slice(0, max);
...
export function resultHasRejectedLocal(aiResult, fittingByName, localRecommendationContext = null) {
  const hasRejected = (rec, allowLocal, entryName = "") => {
    ...
    const acceptsLocal = (modelName) => {
      ...
      return entryLocalNames ? entryLocalNames.has(name) : Boolean(resolveFittingLocalName(name, fittingByName));
    };
    if (!normalized) {
      const provider = String(rec?.provider || "").trim();
  ...
  for (const rec of Array.isArray(aiResult?.cloudRecommendations)
    ...
    if ((norm.fallback_models || []).some((r) => hasRejected(r, true, norm.name)))
      return true;
  }
  ...
  for (const placement of aiResult?.localModels?.placements || []) {
    if (hasRejected({ provider: LOCAL_PROVIDER, model: placement?.modelName }, true, placement?.agentName))
      return true;
  }
  return false;
}
...
/**
 * Formats a run-on AI analysis sentence (particularly fallback rules engine output)
 * into a clean, bulleted layout.
 */
export function formatAiAnalysis(analysis) {
  ...
  // Split prefix into two lines if it contains the full sentence boundary
  ...
  const rawItems = listPart.split(/,\s+/);

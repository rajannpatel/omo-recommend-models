import { writeGroupLine, writeTopLevelLine } from "./progress.js";
...
export function writeNormalSubprocessStatus({
  command = "",
  args = [],
  displayArgs = args,
  inGroup = true,
} = {}) {
  const rendered = `• ${renderCommand(command, displayArgs)}`;
  if (inGroup) {
    writeGroupLine(rendered);
  } else {
    writeTopLevelLine(rendered);
  }
}

...
const CONFIG_PATH = path.join(CONFIG_DIR, "oh-my-openagent.jsonc");
const CACHE_PATH = path.join(CACHE_DIR, "provider-models.json");
const BACKUP_PATH = path.join(CONFIG_DIR, "oh-my-openagent.jsonc.pre-rebalance");
...
function loadProviderModels(options = {}) {
  ...
  if (!fs.existsSync(CACHE_PATH)) {
    ...
    if (!quiet) {
      console.log(
        "⚠ Provider-models cache not found. Refreshing...",
      );
    }
    try {
...
function applyAiRecommendations(section, recommendations) {
  ...
  for (const rec of recommendations) {
    if (rec.model && rec.provider) {
      models.push(`${rec.provider}/${rec.model}`);
    }
  }
  if (models.length > 0) {

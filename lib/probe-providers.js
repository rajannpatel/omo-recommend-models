...
export function probeModel(ctx, modelRef, signal = ctx.signal) {
  ...
  if (ctx.providerProbePromises.has(provider)) {
    return ctx.providerProbePromises.get(provider);
  }
  ...
  const promise = new Promise((resolve) => {
    ...
    child.on("close", (code) => {
      ...
      if (code !== 0 && code !== null) {
        markProviderCreditExhausted(ctx, provider, `exit-code-${code}`);
  ...
  ctx.providerProbePromises.set(provider, promise);
  return promise;
}
...
/**
 * Delegates to ctx.killChild(child).
 */
export function killChild(ctx, child) {
  ctx.killChild(child);
}

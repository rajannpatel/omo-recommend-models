...
export function isQuotaError(_code, text) {
  const lower = String(text || "").toLowerCase();
  return (
    lower.includes("402") ||
...
export function isTrueQuotaExhaustion(_code, text) {
  const lower = String(text || "").toLowerCase();
  return (
    /\bhttp(?:\/\d(?:\.\d)?)?\s+402\b/i.test(lower) ||
    /\bstatus(?:code)?["']?\s*[:=]\s*["']?402\b/i.test(lower) ||
    lower.includes("payment required") ||
    lower.includes("billing limit") ||
    lower.includes("credit limit") ||
    lower.includes("org member budget limit exceeded") ||

...
function resolveProbeClose({ code, timedOut, provider, rawError, ctx }) {
  ...
  if (isModelUnavailableError(rawError)) {
    return { ok: false, reason: "model-unavailable", errorOutput: rawError };
  }
  if (code !== 0 && code !== null) {
...
export function probeModel(ctx, modelRef, signal = ctx.signal, statusFormatter = null) {
  ...
  if (ctx.debugMode && !ctx.verboseMode) {
    writeGroupLine(`[exec] opencode run --pure --agent summary --format json --model ${modelRef}`);
  }
  ...
  const abortHandler = () => {
    ...
    const result = { ok: false, reason: "aborted", errorOutput: "Aborted by user" };
  ...
  child.on("error", (err) => {
    ...
    const result = { ok: false, reason: `spawn-error: ${err.message}` };
    const statusMsg = statusFormatter ? statusFormatter(result) : "";
    if (ctx.verboseMode) {
      reporter.finish(statusMsg);
    } else {

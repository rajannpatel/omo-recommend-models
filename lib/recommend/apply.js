...
function runValidator(validatorPath, stdio = "inherit", configPath = null, ctx = null) {
  const args = configPath ? [validatorPath, "--config", configPath] : [validatorPath];
  execFileSyncWithVerbose(ctx, process.execPath, args, {
    stdio,
...
function writeConfigWithValidation({
  ...
  try {
    runValidator(validatorPath, validateStdio, configPath, ctx);
  } catch (err) {

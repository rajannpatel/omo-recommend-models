 ...
 */
export function discoverCliModels(config = {}, options = {}, ctx, commandExistsFn) {
  ...
  function buildCliInvoker(adapter) {
    return (prompt) => {
      ...
      try {
        const raw = execFileSync(cmd[0], cmd.slice(1), options);

...
export async function rankFallbacksByFitness(cloudRecommendations, cloudLookup = null) {
  const modelMetadata = {};
  if (cloudLookup?.byId) {
  ...
  const header = `◇  processing 0/${total} OmO agent-models and category-models to find the most suitable AI models`;
  ...
  async function tryModel(entry, modelRef) {
    ...
    try {
      ...
      if (result && Array.isArray(result[entry.name]) && result[entry.name].length > 0) {
        process.stdout.write(`\n✓  processed  ${completedCount + 1}/${total} ${label} by ${modelRef}`);
        return { modelRef, ranked: result[entry.name] };
      }
      process.stdout.write(`\n✗  ${label} by ${modelRef} — invalid ranking`);
    } catch (err) {
      const msg = err.message.replace(/^opencode returned /, "");
  ...
  process.stdout.write(`\n│\n◇  AI ranking ${total} agent(s)/category(ies) by model fitness — processed ${total}/${total}│\n│\n`);
  if (rankedCount > 0) {
    const modelList = [...usedModels].map((m) => `│  • ${m}`).join("\n");
    process.stdout.write(`✓  AI ranking ${total}: ${rankedCount}/${total} ranked using\n${modelList}\n`);
  } else {
    process.stdout.write(`◇  AI ranking ${total}: AI unavailable — using heuristic order\n`);
  }

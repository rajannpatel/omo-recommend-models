...
export async function rankFallbacksByFitness(
    ...
    try {
      ...
      if (modelRef.startsWith("cli/")) {
        ...
        text = await callCliAgent(
          buildRankingPrompt(entriesToQuery), 
          tool, 
          config, 
          ctx,
          { debug }
  ...
  for (let attempt = 0; attempt < models.length && remainingEntries.length > 0; attempt++) {
    const modelRef = models[attempt % models.length];
    if (blacklistedModels.has(modelRef)) continue;

    const result = await tryModel(remainingEntries, modelRef);
    const stillRemaining = [];

    for (const entry of remainingEntries) {
      const entryRanking = result?.result?.[entry.name];
      if (Array.isArray(entryRanking) && entryRanking.length > 0) {
        const rec = cloudRecommendations.find((r) => r.name === entry.name);
        if (rec) {
          applyRanking(rec, modelRef, entryRanking, entry.ruleChainMatched);

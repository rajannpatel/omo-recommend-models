...
function resolveAgyPanelModel(ctx) {
  ...
  if (!ctx?.verboseMode) {
    writeNormalSubprocessStatus({ command: "agy", args: ["models"], inGroup: true });
  }
  try {
...
export async function callCliAgent(prompt, tool, config, ctx, options = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (!verbose) {
      writeNormalSubprocessStatus({ command: bin, args, displayArgs, inGroup: true });
    }
    const spawnOptions = {
      stdio: ["ignore", "pipe", "pipe"],
      timeout: 30000,
    ...
    child.on("close", (code) => {
      reporter.finish();
      if (code !== 0) {
        const detail = (verbose || debug) && stderr.trim() ? `: ${stderr.trim().slice(0, 200)}` : "";

...
function probeOpencode(bin, { verbose = false } = {}) {
  return new Promise((resolve) => {
    const args = ["--version"];
    const reporter = createVerboseSubprocessReporter({ enabled: verbose, command: bin, args, inGroup: true });
    if (!verbose) {
      writeNormalSubprocessStatus({ command: bin, args, inGroup: true });
    }
...
async function findOpencode(options = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    child.on("close", (code) => {
      ...
      if (extractedText) {
        ...
        const stderrInfo = (verbose || debug) && stderr.trim()

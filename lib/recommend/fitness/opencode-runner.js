...
export async function callOpencode(prompt, modelRef, ctx = null, options = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (debug) writeGroupLine(`[exec] ${bin} ${args.join(" ")}`);
    const spawnOptions = {
      ...
      for (const line of lines) {
        ...
        try {
          ...
          if (event.type === "text" && event.part?.text) {
            ...
            if (debug) writeGroupLine(`[event:text] ${preview}${preview.length < event.part.text.length ? "..." : ""}`);
          } else if (event.type === "error" && debug) {
    ...
    child.on("close", (code) => {
      ...
      if (extractedText) {
        if (debug) writeGroupLine(`[complete] query ${modelRef}: ${eventCount} events`);

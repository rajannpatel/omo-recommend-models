...
function runDetectionCommand(command, args, options, verbose) {
  const reporter = createVerboseSubprocessReporter({ enabled: verbose, command, args, inGroup: true });
  if (!verbose) {
    writeNormalSubprocessStatus({ command, args, inGroup: true });
  }
  try {
 ...
 */
export function detectGPU({ verbose = false } = {}) {
  const env = dumbEnv();
  try {
    const out = runDetectionCommand(
      "nvidia-smi",
      [
  ...
  try {
    const out = runDetectionCommand("ollama", ["list"], {
      encoding: "utf-8",
      timeout: 5000,
      stdio: ["ignore", "pipe", "pipe"],
      env,
 ...
 */
export function detectOllama(commandExistsFn, { verbose = false } = {}) {
  let installed = false,
    running = false,
    version = null,
    models = [];
  ...
  if (installed) {
    try {
      version = (
        runDetectionCommand("ollama", ["--version"], {
          encoding: "utf-8",
          timeout: 3000,
          stdio: ["ignore", "pipe", "pipe"],
          env,
    ...
    try {
      const list = runDetectionCommand("ollama", ["list"], {
        encoding: "utf-8",
        timeout: 5000,
        stdio: ["ignore", "pipe", "pipe"],
        env,

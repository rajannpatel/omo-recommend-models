...
export async function discoverLocalEnvironment({
  ...
  if (cloudOnlyFlag) {
    ...
    const gpuProgress = createProgress("Checking GPU", { doneSymbol: "◇" });
    gpu = detectGPU();
    gpuProgress.done(gpu.label);

    const ollamaProgress = createProgress("Checking Ollama", { doneSymbol: "◇" });
    ollama = detectOllama();
    ollamaProgress.done(
      ollama.running
  ...
  if (!cloudOnlyFlag && ollama.installed) {
    const localProgress = createProgress("Discovering local model catalog", { doneSymbol: "◇" });

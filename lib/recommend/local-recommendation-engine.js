...
function specialtyForEntry(entryName, entryType) {
  void entryType;
...
function localVramBudgetGb(gpu) {
...
function scoreLocalCandidate(candidate, requirement) {
...
export function buildHardwareDeficitWarning({
  ...
  return {
    ...
    budgetGb,
    scope: "local",
    message: `No fitting local ${requirement.specialty} model for ${requirement.entryName} on ${gpuName} within ${budgetGb}GB budget; lower target context, install a smaller model, use --cloud-only, or upgrade VRAM.`,
  };
}

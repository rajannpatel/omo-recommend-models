...
export function mergeAndDedupe(matches) {
  ...
  // Keep 1 best-scored model per provider. Multiple models from the same
  // provider in fallback_models are redundant — the user only wants the
  // best model from each configured provider (except opencode which has
  // special pool handling in withMinimumFreeFallbacks).
...
export function sortByPreference(matches, providerPreferenceOrder) {
  ...
  return matches.sort((a, b) => {
    ...
    // Same provider (or both not in preference list):
    // higher score first so the best model becomes primary
    if (a.score !== b.score) {
      return (b.score || 0) - (a.score || 0);
    }
    
    // Then by provider name for deterministic ordering

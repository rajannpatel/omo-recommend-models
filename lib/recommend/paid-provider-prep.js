...
function paidModelRefsFromLookup(cloudLookup) {
  const refs = [];
...
function probeModelRefsFromLookup(cloudLookup) {
  const refs = [];
  for (const [provider, modelMap] of Object.entries(cloudLookup.byId || {})) {
...
function providerProbeCandidates(sortedPaidRefs) {
  const byProvider = {};
...
function createProbeAwaiter({ paidProbeProgress, paidProbesEnabled, paidProbesPromise }) {
...
export function preparePaidProviderModels({
  ...
  if (shouldProbeProviderAvailability(ctx)) {
    ...
    if (providers.length > 0) {
      process.stdout.write(`◇  Verifying availability for ${providers.length} cloud provider(s) — this may take ~30s...\n│\n`);
      paidProbeProgress = createProgress("Verifying cloud models availability", {

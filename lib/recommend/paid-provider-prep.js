...
import { shouldProbeProviderAvailability, isProviderAvailable } from "../probe-providers.js";
import {
...
export async function preparePaidProviderModels({
  ...
  const excludedSet = new Set((excludeModels || []).map(m => String(m).trim().toLowerCase()));
  const filteredEligibleRefs = eligibleRefs.filter((ref) => {
    const slash = ref.indexOf("/");
    const provider = slash === -1 ? ref : ref.slice(0, slash);
    if (excludedSet.has(provider.toLowerCase()) || excludedSet.has(ref.toLowerCase())) {
      return false;
    }
  ...
  if (shouldProbeProviderAvailability(ctx)) {
    const probeResults = await runProviderProbes({
      cloudLookup: initialCloudLookup,
      ctx,
      eligibleRefs: filteredEligibleRefs,
      probeModelFn,
    });

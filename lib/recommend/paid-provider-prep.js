...
export function preparePaidProviderModels({
  ...
  if (shouldProbeProviderAvailability(ctx)) {
    ...
    const totalModels = providers.reduce(
      (total, provider) => total + (byProvider[provider]?.length || 0),
      0,
    );
    ...
    const updateProgressMessage = () => {
      ...
      const activeList = activeSnapshot
        .map(([prov, info]) => {
          ...
          return `${prov} (${modelName} - model ${info.index}/${info.total})`;
        })
        .join(", ");
    ...
    const probePromises = providers.map(async (provider) => {
      ...
      try {
        ...
        for (let idx = 0; idx < models.length; idx++) {
          ...
          activeProbes.set(provider, { modelRef, index: idx + 1, total: models.length });
        ...
        if (lastResult?.reason === "quota-exceeded") {
        ...
        paidProbeProgress?.advance(
          checkedModels,
          checkedModels > 0 ? `${provider} checked` : `${provider} skipped`,
        );
      }
    });

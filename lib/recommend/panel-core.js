...
export function buildAgentPrompt(entry, cloudLookup, allLocalModels, gpu, ollama, ctx) {
  ...
  const rankedContextCandidates = hasContextRanking
  ...
  const localTable = fitting
    .map(
      (m) => {
        const totalVramGb = m.totalVramGb ?? m.vram;
        const weightGb = m.weightGb ?? "?";
        const kvCacheGb = m.kvCacheGb ?? "?";
        const installed = typeof m.installed === "boolean" ? m.installed : installedNames.has(m.name);
        return `${m.name}  total=${totalVramGb ?? "?"}GB weight=${weightGb}GB kv=${kvCacheGb}GB score=${m.score ?? 0} ${installed ? "inst" : "missing"}`;
      },
    )
    .join("\n");
...
export async function runPanelAndSelect(
  ...
  void cloudOnlyFlag;
  const cliAgents = discoverCliModelsFromAgents(
    config,
    cliOptions,
    ctx,
    commandExists,
  ...
  if (panelModels && panelModels.length > 0) {
    ...
    models = defaultPanelModelsFn(config, cloudLookup, cliOptions);
...

  {
    ...
    let probeResults = [];
    try {
      probeResults = await Promise.all(models.map(async (m) => {
        const cliAgent = findCliAgent(cliAgents, m);
        return cliAgent ? await cliAgent.probe() : probeModel(ctx, m);
      }));
      availableModels = models.filter((_model, i) => probeResults[i].ok);
    ...
    if (availableModels.length === 0) {
      ...
      try {
        const probeResults2 = await Promise.all(freeModels.map(async (m) => {
          const cliAgent = findCliAgent(cliAgents, m);
          return cliAgent ? await cliAgent.probe() : probeModel(ctx, m);
        }));
        availableModels = freeModels.filter((_model, i) => probeResults2[i].ok);
  ...
  const updateStatus = () => {
    if (process.stdout.isTTY) {
      ...
      process.stdout.write(`\x1b[2K${formatLinePrefix("started")}${formatCount(tasksStarted, totalTasks)}\n`);
  ...
  console.log(`${formatLinePrefix("started")}${formatCount(tasksStarted, totalTasks)}`);
  ...
  for (let ai = 0; ai < agents.length; ai++) {
    ...
    for (const m of models) {
      taskFns.push(async () => {
        const modelLabel = m.startsWith("cli/") ? m : m.split("/").pop();
        currentAgent = `${entry.name} with ${modelLabel}`;
        tasksStarted++;
        if (!process.stdout.isTTY) {
          console.log(`evaluating ${currentAgent}`);
        }
        updateStatus();
        const statusRef = {};

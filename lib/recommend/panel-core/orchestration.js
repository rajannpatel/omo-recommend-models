...
async function verifiedModels(models, cliAgents, ctx, label) {
  const progress = createProgress(label, { total: models.length });
  let probeResults = [];
  try {
    const indexedResults = await Promise.all(models.map(async (model, index) => {
      const cliAgent = findCliAgent(cliAgents, model);
      try {
        const result = cliAgent ? await cliAgent.probe() : await probeModel(ctx, model);
        return { index, result };
      } finally {
        progress?.advance();
      }
    ...
    probeResults = indexedResults.map((entry) => entry.result);
  ...
  for (let agentIndex = 0; agentIndex < agents.length; agentIndex++) {
    ...
    for (const model of models) {
      tasks.push(async () => {
        ...
        const statusRef = {
          onQueryProgress(current, total, detail) {
            panelStatus.queryProgress = {
              current,
              detail: `${panelStatus.currentAgent} ${detail}`,
              total,
            };
            if (!process.stdout.isTTY) {
              console.log(`AI Panel query attempts: ${current}/${total} ${panelStatus.queryProgress.detail}`);
            }
            updateStatus(panelStatus, agents, models);
          },
        };
        ...
        if (!process.stdout.isTTY) {
          console.log(`AI Panel evaluations: ${formatProgressCount(panelStatus.tasksDone, panelStatus.totalTasks)}`);
        }

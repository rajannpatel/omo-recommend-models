import { formatProgressCount } from "../../display-utils.js";
...
export function createPanelStatus(agents, models) {
  ...
  const countWidth = String(Math.max(totalTasks, agents.length, 3)).length;
...
export function printInitialStatus(agents, models, panelStatus) {
  ...
  console.log("AI Panel query attempts: -");
  console.log(`AI Panel evaluations: ${formatProgressCount(0, panelStatus.totalTasks)}`);
...
export function updateStatus(panelStatus, agents, models) {
  ...
  process.stdout.write(`\x1b[${models.length + 6}F`);
  for (const model of models) {
  ...
  process.stdout.write(`\x1b[2KAI Panel query attempts: ${formatQueryProgress(panelStatus)}\n`);
  process.stdout.write(`\x1b[2KAI Panel evaluations: ${formatProgressCount(panelStatus.tasksDone, panelStatus.totalTasks)}\n`);
...
function formatQueryProgress(panelStatus) {
  if (!panelStatus.queryProgress) return "-";
  const { current, total, detail } = panelStatus.queryProgress;

...
export async function selectPanelRecommendation(env) {
  ...
  if (env.cliOptions.parallelPanel) {
    ...
    const tier1 = panelModels.filter((m) => m.startsWith("opencode/") || m === "cli/opencode");
    const tier2 = panelModels.filter((m) => m === "cli/agy");
    const tier3 = panelModels.filter((m) => m === "cli/codex");
    const otherModels = panelModels.filter((m) => !tier1.includes(m) && !tier2.includes(m) && !tier3.includes(m));
    tier2.push(...otherModels);

    const hasSuccessfulResponse = (res) => {
      return res && res.panel && res.panel.state && res.panel.state.some((s) => s.results.some((r) => r !== null));
    };

    if (tier1.length > 0) {
      const res = await env.runPanelAndSelect(tier1);
    ...
    if (!panelResult && tier2.length > 0) {
      const res = await env.runPanelAndSelect(tier2);
      if (hasSuccessfulResponse(res)) {
        panelResult = res;
        chosenModels = tier2;
      }
    ...
    if (!panelResult && tier3.length > 0) {
      const res = await env.runPanelAndSelect(tier3);
      if (hasSuccessfulResponse(res)) {
        panelResult = res;
        chosenModels = tier3;
      }

...
const MIN_DEFAULT_PROBE_GLOBAL_CONCURRENCY = 2;

function positiveInteger(value) {
  return Number.isInteger(value) && value > 0 ? value : null;
}

function detectedParallelism(readCount) {
  try {
    return positiveInteger(readCount());
  } catch {
    return null;
  }
}

// Exported for tests: takes an `os`-shaped param so callers can inject a
// fake without Node's `mock.module` (requires Node 20.1+; package.json
// only guarantees >=18.0.0, where `os.availableParallelism` may also be
// absent - hence the cpus().length fallback below).
export function detectDefaultProbeGlobalConcurrency(osModule = os) {
  const available = typeof osModule.availableParallelism === "function"
    ? detectedParallelism(() => osModule.availableParallelism())
    : null;
  const cpuCount = available === null && typeof osModule.cpus === "function"
    ? detectedParallelism(() => osModule.cpus()?.length)
    : null;
  // The floor is a fallback for when detection itself is unusable, not a
  // minimum imposed on a genuinely detected value - a real 1-core host
  // must get global:1, not be pushed above its actual hardware capacity.
  return available ?? cpuCount ?? MIN_DEFAULT_PROBE_GLOBAL_CONCURRENCY;
}

const DEFAULT_PROBE_CONCURRENCY = Object.freeze({
  global: detectDefaultProbeGlobalConcurrency(),
  perProvider: 2,
  freeOrLocal: 1,
});
...
async function executeProbeSequence({
  ...
  const quotaExhaustedProviders = new Set();
        ...
        Promise.resolve(probeModelFn(ctx, task.parsed.modelRef, ctx.signal))
          .then((result) => {
            ...
            if (result?.scope === "provider" && result.reason === "quota-exceeded") {
              ...
              quotaExhaustedProviders.add(task.parsed.provider);
              markQueuedProviderClosed(task.parsed.provider, "quota-exceeded");
            }
            if (record.reason === "rate-limited") {
              markProviderRateLimited(
                ctx,
                task.parsed.provider,
                parseRetryAfterSeconds(result?.errorOutput),
                "rate-limited",
              );
              markQueuedProviderClosed(task.parsed.provider, "rate-limited");
            }
            if (quotaExhaustedProviders.has(task.parsed.provider)) {
...
export async function runProviderProbes({
  ...
  const paidProbesPromise = probeRecordsPromise.then((records) =>
    records
      .filter((record) => record.outcome === "available" && isProviderAvailable(ctx, record.provider))

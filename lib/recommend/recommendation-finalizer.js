...
function normalizedCloudRecommendations(completed, fittingByName, cloudLookup, isProviderAllowed) {
  ...
  for (const rec of Array.isArray(completed.cloudRecommendations) ? completed.cloudRecommendations : []) {
    ...
    normalizeRuntimeRefs(norm, fittingByName, cloudLookup, isProviderAllowed);
    recByName.set(norm.name, norm);
  }
  return recByName;
}

function normalizeRuntimeRefs(rec, fittingByName, cloudLookup, isProviderAllowed) {
  rec.model = normalizeLocalRecommendation(rec.model, fittingByName, true);
  if (
    rec.model &&
    (!isProviderAllowed(rec.model.provider) || !isUsableForConfig(rec.model, cloudLookup))
  ) {
    rec.model = null;
  }
  rec.routing = (rec.routing || [])
    .map((ref) => normalizeLocalRecommendation(ref, fittingByName, false))
    .filter((ref) => ref && isProviderAllowed(ref.provider) && isUsableForConfig(ref, cloudLookup));
  rec.fallback_models = (rec.fallback_models || [])
    .map((ref) => normalizeLocalRecommendation(ref, fittingByName, true))
    .filter((ref) => ref && isProviderAllowed(ref.provider) && isUsableForConfig(ref, cloudLookup));
}

function cloudProvidersForFallback(cloudLookup, isProviderAllowed) {
...
function addMissingCloudFallbacks(rec, providerSets, cloudProviders, cloudLookup) {
  for (const [provider, modelMap] of cloudProviders) {
    if (provider === rec.model?.provider || providerSets.fallbackProviders.has(provider)) {
      continue;
    }
    const fallback = bestCloudRecommendationForProvider(provider, modelMap, cloudLookup);
...
export function completeAiRecommendations(
  ...
  const cloudProviders = cloudProvidersForFallback(cloudLookup, isProviderAllowed);

  for (const entry of entries) {
    const rec = baseRecommendationForEntry(entry, recByName);
    normalizeRuntimeRefs(rec, fittingByName, cloudLookup, isProviderAllowed);
    ...
    addMissingCloudFallbacks(rec, providerSets, cloudProviders, cloudLookup);
    addLocalFallback({
      allLocalModels,
      completed,
      entry,

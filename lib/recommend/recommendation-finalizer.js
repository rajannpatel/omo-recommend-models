...
import { buildFreeModelRefPredicate, isFreeModelRef, isZeroCostModelMeta } from "../shared/provider-cache.js";
...
/**
 * Ensures each provider contributes at most one non-free and one free model
 * across model + fallback_models.
 *
...
export function deduplicatePerProvider(rec, { isFreeRef = (ref) => {
  ...
  const usedFreeProviders = new Set();
  const usedPaidProviders = new Set();
  if (rec.model?.provider) {
    const usedProviders = isFreeRef(rec.model) ? usedFreeProviders : usedPaidProviders;
    usedProviders.add(rec.model.provider);
  }
  rec.fallback_models = (rec.fallback_models || []).filter((ref) => {
    if (!ref?.provider) return false;
    const usedProviders = isFreeRef(ref) ? usedFreeProviders : usedPaidProviders;
...
export function completeAiRecommendations(
  ...
  for (const [provider, modelMap] of Object.entries(cloudLookup?.byId || {})) {
    ...
    for (const [modelId, meta] of modelMap.entries()) {
      if (isZeroCostModelMeta(meta)) {
        freeModelRefs.add(`${provider}/${modelId}`);
        if (meta.capabilities?.toolcall === true) {
          freeModelCandidates.push({
            provider,
            model: modelId,
            reason: "Free cloud fallback",
            score: scoreModel(`${provider}/${modelId}`, null, meta),
          });
        }
      }
    }
  }
  freeModelCandidates.sort((a, b) => (b.score - a.score) || a.provider.localeCompare(b.provider) || a.model.localeCompare(b.model));
  const costBasedIsFreeRef = buildFreeModelRefPredicate(cloudLookup);
  const isFreeRef = (ref) => {
    if (!ref?.provider || !ref?.model) return false;
    if (freeModelRefs.size > 0) return costBasedIsFreeRef(ref);
    return isFreeModelRef(ref.provider, ref.model);
  };

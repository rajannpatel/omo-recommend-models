...
import { buildProviderPreferenceOrder } from "./rules-assignment/helpers.js";
...
/**
 * Sorts fallback_models by provider preference order.
 */
function sortByProviderPreference(fallbackModels) {
  const preferenceOrder = buildProviderPreferenceOrder();
  return [...fallbackModels].sort((a, b) => {
...
function finalizeEntryRecommendation(rec) {
  ...
  // Promote the first fallback to model BEFORE sorting by preference,
  // so the sort doesn't move a different model ahead of the one
  // the pipeline intended as the primary alternative.
  if (!rec.model && rec.fallback_models.length > 0) {
    rec.model = rec.fallback_models.shift();
  }
  rec.fallback_models = sortByProviderPreference(rec.fallback_models);

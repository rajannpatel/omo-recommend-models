...
export function completeAiRecommendations(
  ...
  for (const entry of entries) {
    ...
    for (const [provider, modelMap] of Object.entries(cloudLookup?.byId || {})) {
      ...
      for (const [modelId, meta] of modelMap.entries()) {
        const isFree = meta && typeof meta.pricing === "object"
          ? meta.pricing?.input === 0 && meta.pricing?.output === 0 && meta.capabilities?.toolcall === true
          : isFreeModelRef(provider, modelId);
        if (!isFree) continue;
        const candidate = { provider, model: modelId, reason: "Free cloud fallback" };
        if (!isModelAllowed(candidate)) continue;
        if (rec.fallback_models.some((f) => f.provider === provider && f.model === modelId)) continue;

...
import { mergeAndDedupe, sortByPreference } from "./model-matching.js";
...
import { isZeroCostModelMeta } from "../shared/provider-cache.js";

function withCostClassification(candidate, cloudLookup) {
  const meta = cloudLookup?.byId?.[candidate.provider]?.get(candidate.model);
  if (!meta) return candidate;
  return { ...candidate, isFree: isZeroCostModelMeta(meta) };
}
...
export function createRuleBasedRecommendations({
  ...
  for (const entry of allConfigEntries(config)) {
    ...
    const [primary, ...fallbacks] = candidates
      .map((candidate) => withCostClassification(candidate, cloudLookup))
      .map(withReason);

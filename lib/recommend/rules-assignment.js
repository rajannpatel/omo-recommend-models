...
export function createRuleBasedRecommendations({
  ...
  for (const entry of allConfigEntries(config)) {
    ...
    if (candidates.length === 0) {
      ...
      const chain = expandChain(requirement);
      const groups = [];
      ...
      for (const c of chain) {
        if (c.model !== currentModel) {
          ...
          currentProviders = [c.provider];
        } else {
          if (!currentProviders.includes(c.provider)) {
            currentProviders.push(c.provider);
          }
        }
      }
      if (currentModel !== null) {
      ...
      const triedList = groups
        .map((g) => {
          if (g.providers.length > 1) {
            return `(${g.providers.join(", ")})/${g.model}`;
          }
          return `${g.providers[0]}/${g.model}`;
        })
        .join(" > ");
      unresolved.push(`${entry.name} (tried: ${triedList})`);

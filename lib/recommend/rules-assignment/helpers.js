...
import { normalizeModelNameForMatching } from "../../omo-shared.js";
...
export function findActualModel(modelMap, model) {
  ...
  const wanted = normalizeModelNameForMatching(model);
  for (const id of modelMap.keys()) {
    if (normalizeModelNameForMatching(id) === wanted) {
      return id;
    }
  }
  return null;
...
export function expandChain(requirement) {
  const out = [];

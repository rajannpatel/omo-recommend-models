...
import { normalizeModelNameForMatching } from "./model-refs.js";
...
export function filterAccessibleModels(models, accessible) {
  ...
  const parsedAccessible = [];
  for (const item of accessible) {
    const slash = item.indexOf("/");
    if (slash !== -1) {
      parsedAccessible.push({
        provider: item.slice(0, slash),
        model: item.slice(slash + 1),
        original: item
      });
    } else {
      parsedAccessible.push({
  ...
  for (const [providerId, modelsArray] of Object.entries(models)) {
    ...
    for (const model of modelsArray) {
      ...
      const wantedNormalized = normalizeModelNameForMatching(id);
      let foundFuzzy = null;
      for (const acc of parsedAccessible) {
        if (acc.provider && acc.provider !== providerId) {
          continue;
        }
        if (normalizeModelNameForMatching(acc.model) === wantedNormalized) {
          foundFuzzy = acc.model;
          break;
        }
      }

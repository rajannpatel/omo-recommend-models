...
} from "./openrouter-policy.js";
import { createVerboseSubprocessReporter, writeNormalSubprocessStatus } from "../display/subprocess-output.js";
import { writeGroupLine } from "../display/progress.js";


let _cachedFreeModels = null;
let _cachedProviderModels = null;
let _cachedProviderModelsKey = null;

export function resetCache() {
  _cachedFreeModels = null;
  _cachedProviderModels = null;
  _cachedProviderModelsKey = null;
}
...
export function isFreeModelRef(provider, model) {
  ...
  try {
    ...
    if (catalog && catalog[provider]?.models?.[model]) {
      return isZeroCostModelMeta(catalog[provider].models[model]);
    }
  } catch (e) {}
...
export function isZeroCostModelMeta(meta) {
  const cost = meta?.cost || meta?.pricing;
  return cost?.input === 0 && cost?.output === 0;
}

export function buildFreeModelRefPredicate(cloudLookup) {
  ...
  for (const [provider, modelMap] of Object.entries(cloudLookup?.byId || {})) {
    ...
    for (const [modelId, meta] of modelMap.entries()) {
      if (isZeroCostModelMeta(meta)) freeModelRefs.add(`${provider}/${modelId}`);
    }
  }
  return (ref) => {
    if (!ref?.provider || !ref?.model) return false;
    return freeModelRefs.has(`${ref.provider}/${ref.model}`);
  };
}
...
export function discoverFreeModels(options = {}) {
  ...
  if (freeModels.length === 0) {
    ...
    if (!options.ctx?.verboseMode) {
      writeNormalSubprocessStatus({ command: "opencode", args: ["models", "opencode"], inGroup: true });
    }
    try {
    ...
    (models || [])
      ...
      .filter((model) => {
        ...
        if (catalog && catalog[provider]?.models?.[id]) {
          ...
          const caps = modelData.capabilities || {};
          return isZeroCostModelMeta(modelData) && caps.toolcall === true;
        }
        return false;
      }),
  )];
...
function getAccessibleModels(options = {}) {
  const reporter = createVerboseSubprocessReporter({
    enabled: options.ctx?.verboseMode,
    command: "opencode",
    args: ["models"],
  ...
  if (!options.ctx?.verboseMode) {
    writeNormalSubprocessStatus({ command: "opencode", args: ["models"], inGroup: true });
  }
  try {
    ...
    if (options.ctx?.verboseMode || options.ctx?.debugMode) {
...
function parseVerboseModels(output) {
  ...
  for (const line of lines) {
    ...
    if (!jsonStr && !line.trim().startsWith("{")) continue;
    jsonStr += `${line}\n`;
    braceDepth += jsonBraceDelta(line);
    if (braceDepth === 0 && jsonStr.trim()) {
      try {
...
function enrichModelsFromVerbose(liveModels, verboseProviderModels) {
...
export async function loadProviderModels(options = {}) {
  const cachedPolicyRefs = options.ctx?.policyExclusionCache?.values?.() ?? [];
  const providerModelsKey = `${openRouterPolicyCacheIdentity(options.env)}:${cachedPolicyRefs.join("\0")}`;
  if (_cachedProviderModels && _cachedProviderModelsKey === providerModelsKey) return _cachedProviderModels;
  ...
  const openrouterUserModelIds = models.openrouter
    ? await fetchOpenRouterUserModelIds(options.env)
    : null;
  if (openrouterUserModelIds) {
    const openrouterCount = models.openrouter.length;
    models.openrouter = models.openrouter.filter((modelId) => openrouterUserModelIds.has(modelId));
    const excludedCount = openrouterCount - models.openrouter.length;
    const noun = excludedCount === 1 ? "model" : "models";
    writeGroupLine(`• OpenRouter policy configuration: excluded ${excludedCount} ${noun} before probes`);
  } else if (models.openrouter) {
    writeGroupLine("• OpenRouter policy configuration: unavailable; checking cached exclusions before probes");
  }

  if (models.openrouter && options.ctx?.policyExclusionCache) {
    const openrouterCount = models.openrouter.length;
    models.openrouter = models.openrouter.filter(
      (modelId) => !options.ctx.policyExclusionCache.has(`openrouter/${modelId}`),
    );
    const excludedCount = openrouterCount - models.openrouter.length;
    const noun = excludedCount === 1 ? "model" : "models";
    writeGroupLine(`• OpenRouter cached policy exclusions: excluded ${excludedCount} ${noun} before probes`);
  }
  ...
  if (!options.ctx?.verboseMode) {
    writeNormalSubprocessStatus({ command: "opencode", args: ["models", "--verbose"], inGroup: true });
  }
  let verboseOutput;
  try {
    ...
    for (const [providerId, liveModels] of Object.entries(models)) {
      ...
      const enriched = enrichModelsFromVerbose(liveModels, verboseProviderModels);
  ...
  _cachedProviderModels = { models, advertisedRefs: [...accessible] };
  _cachedProviderModelsKey = providerModelsKey;

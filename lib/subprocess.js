...
export function execFileSyncWithVerbose(ctx, command, args, options = {}) {
  ...
  const normalInGroup = options.inGroup ?? true;
  const reporter = createVerboseSubprocessReporter({ enabled: verbose, command, args, displayArgs, inGroup: options.inGroup });
  try {
    if (verbose) {
    ...
    writeNormalSubprocessStatus({ command, args, displayArgs, inGroup: normalInGroup });
    ...
    if (!verbose) {
      throw sanitizeSubprocessError(command, error);
    }
    throw error;
  } finally {
    reporter.finish();
...
function sanitizeSubprocessError(command, error) {
  const status = error?.status ?? error?.code ?? error?.signal;
  const suffix = status === undefined ? "" : ` (${String(status)})`;
  const sanitized = new Error(`Command failed: ${command}${suffix}`);
  sanitized.cause = error;
  sanitized.status = error?.status;
  sanitized.signal = error?.signal;
  sanitized.code = error?.code;
  sanitized.stdout = error?.stdout;
  sanitized.stderr = error?.stderr;
  return sanitized;
}
...
export class SubprocessRunner {
  ...
  execAsync(command, args, options = {}) {
    ...
    return new Promise((resolve) => {
      ...
      if (!this.ctx.verboseMode) {
        writeNormalSubprocessStatus({
          command,
          args,
          displayArgs: options.displayArgs || args,
          inGroup: options.inGroup ?? true,
        });
      }
  ...
  fetchUrl(url, accept) {
    try {
      ...
      args.push(url);
      return this.execSync("curl", args, { timeout: 15000, inGroup: true });
    } catch {
      return "";
    }
  }
  ...
  async fetchUrlAsync(url, accept) {
    try {
      ...
      if (accept) args.push("-H", accept);
      args.push(url);
      const result = await this.execAsync("curl", args, { totalTimeoutMs: 15000, inGroup: true });
      return result || "";
    } catch {
      return "";
    }
